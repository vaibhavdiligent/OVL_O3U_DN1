...
FORM get_ref_delivery_network USING delivery_network
  ...
  DATA:  temp_dn_tb LIKE oiu_pr_dn,
         not_found   TYPE c.
...
ENDFORM.                    " GET_REF_DELIVERY_NETWORK
*&                                                                     *
...
*        >P_W_MESSAGE_DS text
*                                                                      *
*FORM set_error_message USING    p_message LIKE bapiret2.
*
* IF p_message-type = c_messagetype_e.
*    MESSAGE ID p_message-id
*    TYPE       p_message-type
*    NUMBER     p_message-number
*    WITH       p_message-message_v1
...
FORM prepare_create_for_save.
    ...
    TRANSLATE roiu_pr_dn-shtxt TO UPPER CASE.            "#EC TRANSLANG
...
ENDFORM.                    " PREPARE_CREATE_FOR_SAVE
*&                                                                        *
...
FORM prepare_update_for_save.
  ...
  roiu_pr_dn-xxdbaction = c_update_act.                  "SOGK026707 BT
  roiu_pr_dn-shtxt = roiu_pr_dn-dn_de.
  SET LOCALE LANGUAGE sy-langu.
  TRANSLATE roiu_pr_dn-shtxt TO UPPER CASE.              "#EC TRANSLANG
  SET LOCALE LANGUAGE space.
...
ENDFORM.                    " PREPARE_UPDATE_FOR_SAVE
*&                                                                        *
...
*        >P_TEXT_005 text
*                                                                    *
FORM popup_confirm_step USING     text1
                                  text2
                                  title.
...
* <     p2        text
*                                                                    *
FORM change_document USING dn1   STRUCTURE roiu_pr_dn
                           dn2   STRUCTURE roiu_pr_dn.
...
*      EXPORTING
*           nr_range_nr        = oiu_pr_dn_grp-dn_nrnr
*           object             = c_num_rng_obj
**        SUBOBJECT          = ' '
**        TOYEAR             = '0000'
*      IMPORTING
*           interval           = w_interval_ds
*      EXCEPTIONS
...
*           OTHERS             = 3.
  ...
  CALL FUNCTION 'OIU_NUMBER_GET_INFO'
       EXPORTING
            tabname            = g_tabname
            externind          = oiu_pr_dn_grp-externind
...
FORM check_for_validity.
  ...
  DATA:    del_number TYPE i,
           l_dn_grp_tx LIKE oiu_pr_dn_grp_tx-dn_grp_tx.
    CONSTANTS:    c_internal(2)   TYPE c   VALUE '02'.
...
*       and ROIU_PR_DN-DN_NO IS INITIAL.
  ELSEIF w_interval_ds-externind = c_true.
    IF    roiu_pr_dn-dn_no IS INITIAL
       OR ( ( w_interval_ds-fromnumber GT roiu_pr_dn-dn_no )"#EC PORTABLE
       OR   ( w_interval_ds-tonumber   LT   roiu_pr_dn-dn_no ) ).
...
*         EXPORTING
*              nr_range_nr             = w_interval_ds-nrrangenr
*              object                  = w_interval_ds-object
*              quantity                = '1'
**             SUBOBJECT               = ' '
**             TOYEAR                  = '0000'
**             IGNORE_BUFFER           = ' '
*         IMPORTING
*              number                  = roiu_pr_dn-dn_no
**             QUANTITY                =
**             RETURNCODE              =
*         EXCEPTIONS
*              interval_not_found      = 1
...
ENDFORM.                    " GET_DEL_NETWORK_NUMBER
*&                                                                       *
...
* When DN type is 'P' and company code or joint venture are blank error
  IF roiu_pr_dn-type_cd EQ c_gas_plant
    ...
    OR    roiu_pr_dn-vname IS INITIAL ).
    CLEAR w_fcode.
    PERFORM set_return_message USING c_messagetype_e
                                      c_messageid_cm
                                      '009'
...
FORM check_valid_gasplant_ntwk_id.
  ...
  DATA:    temp_gplt_dn_tb LIKE oiu_pr_dn.
...
ENDFORM.                    " CHECK_VALID_GASPLANT_NETWORK_I
*&                                                                      *
...
FORM check_country_and_region.
  ...
  CALL FUNCTION 'OIU_VERIFY_PRIMARY_GEO_LOC'
       EXPORTING
            ...
            pri_loc_code         = roiu_pr_dn-pri_geo_loc
       IMPORTING
  ...
  IF w_geo_required_fg = c_true.
    CALL FUNCTION 'OIU_VERIFY_SECONDARY_GEO_LOC'
       EXPORTING
            land1                 = roiu_pr_dn-cntry_cd
            pri_loc_code          = roiu_pr_dn-pri_geo_loc
            sec_loc_code          = roiu_pr_dn-sec_geo_loc
       EXCEPTIONS
...
ENDFORM.                    " CHECK_COUNTRY_AND_REGION
*&                                                                    *
...
*          _COLLECT       = ' '
              .
ENDFORM.                     " UNLOCK_DELIVERY_NUMBER
*&                                                                    *
...
*         >P_C_CREATE text
*                                                                     *
FORM leave_to_transaction USING     mode.
...
FORM use_long_text_processing USING long_text_read_fg
  ...
  DATA: long_text_name     LIKE thead-tdname.
  ...
  IF long_text_read_fg = c_flag_off.
    ...
    CALL FUNCTION 'OIU_LONG_TEXT_MAINTAIN'
         EXPORTING
              oiu_flg_only_check   = c_flag_on
              oiu_modus            = w_new_mode
              oiu_table_name       = c_dn_tab_name
              oiu_text_id          = c_ltxt
              oiu_text_name        = long_text_name
*             OIU_TEXT_NAME_REF    = '              '
         IMPORTING
...
ENDFORM.                    " USE_LONG_TEXT_PROCESSING
*&                                                                      *
...
************************************************************************
    IF oiu_cm_lxpgl_tx-pri_geo_loc NE roiu_pr_dn-pri_geo_loc.
      SELECT SINGLE * FROM oiu_cm_lxpgl_tx
                      ...
                      AND   land1        = roiu_pr_dn-cntry_cd
                      AND   pri_geo_loc = roiu_pr_dn-pri_geo_loc.
    ENDIF.
  ELSE.
...
************************************************************************
    SELECT SINGLE * FROM oiu_cm_lxsgl_tx
                    ...
                    AND   land1       = roiu_pr_dn-cntry_cd
                    AND   pri_geo_loc = roiu_pr_dn-pri_geo_loc
                    AND   sec_geo_loc = roiu_pr_dn-sec_geo_loc.

    ELSE.
      CLEAR: oiu_cm_lxsgl_tx-sec_geo_desc.
    ENDIF.

ENDFORM.                    " GET_COUNTRY_TEXTS
*&                                                                     *
...
FORM get_typecd_texts.
  ...
  DATA: domain_value   LIKE dd07v-domvalue_l.
...
FORM save_successful.
  ...
  DATA: f_dnno    LIKE roiu_pr_dn-dn_no,
        f_dnde    LIKE roiu_pr_dn-dn_de,
        f_dnal    LIKE roiu_pr_dnvd-al_fl,
        f_dnrc(2) TYPE c,
            f_dnid(8) TYPE c     VALUE 'OIUGT_DN'.
...
FORM translate_texts.
    ...
    DATA:     descr    LIKE roiu_pr_dn-dn_de.

    descr = roiu_pr_dn-dn_de.
    TRANSLATE roiu_pr_dn-dn_de TO UPPER CASE.                 "#EC TRANSLANG
    roiu_pr_dn-dn_de = descr.

ENDFORM.                    " TRANSLATE_TEXTS
*&                                                                             *
...
ENDFORM.                     " NUMBER_RANGE_CHECK_BAPI
*&                                                                      *
...
ENDFORM.                       " CHECK_ALLOCATION_TOLERANCE
...
ENDFORM.                      " BAPI_EDIT_CHECKS_CHANGE
*&                                                                     *
...
FORM set_parameter_id.
  ...
  ELSE.
    ...
    CLEAR setheadert-descript.           "SO7K011423 BT
...
* Check for Razorfish entry for internal Number range     "N 1736401
...
* hierarchy update called separately
    IF SY-SUBRC = 0.                                     "SOGK027671 VV
     Perform F_UPDATE_HIERARCHY.                         "SOGK027671 VV
     COMMIT WORK.                                        "SOGK027671 VV
    ENDIF.                                               "SOGK027671 VV
...
ENDFORM.                    " CREATE_DELIVERY_NETWORK
*&                                                                        *
...
ENDFORM.                    " UPDATE_DELIVERY_NETWORK
*&                                                                       *
...
ENDFORM.                    " LEAVE_TO_DN_DATED_TRANS
*&                                                                     *
...
*                                                                      *
*        >P_SET    SET NAME ENTERED BY THE USER ON SCREEN
*                                                                      *
FORM check_setid USING    set LIKE roiu_pr_dn-dn_set.
...
*      check the type
    ...
    setheadert-descript = ls_setinfo-title.     " SO7K011423 BT
...
*                                                                        *
FORM check_dn_setid USING      set LIKE roiu_pr_dn-dn_set.
   IF set IS INITIAL.
     PERFORM set_return_message USING c_messagetype_e
                                       c_messageid_cm
                                       '008'
...
*                                                                       *
*    > OLD_SET    Old Set ID from Record (Create = Blank)
*    > NEW_SET    New Set ID from Record
*                                                                       *
...
* is the Hier. active?
  m_hier_active.                                         " SOGK013053 EY
* If "NO" then skip the hierarchy update.
  IF w_hier_active IS INITIAL.                           " SOGK013053 EY
    EXIT.                                               " SOGK013053 EY
  ENDIF.                                                 " SOGK013053 EY
...
*         >P_L_DN_GRP_TX text
*                                                                     *
FORM get_dn_grp_tx USING     p_dn_grp
                             p_dn_grp_tx.

  IF NOT p_dn_grp IS INITIAL.
    SELECT SINGLE dn_grp_tx
    INTO   p_dn_grp_tx
    FROM   oiu_pr_dn_grp_tx
...
FORM default_hier_node.
   DATA: l_single_dn_set TYPE setidlist,
         ...
         l_line_count    LIKE sy-index.             "SOGK005579 BT

  CALL FUNCTION 'OIU_SET_STD_HIER_NODES'
       EXPORTING
            setclass     = c_setclass
       IMPORTING
            single_set   = l_single_dn_set
            line_count   = l_line_count "SOGK005579 BT
       EXCEPTIONS
            default_node = 1
            OTHERS       = 2.
  ...
  CASE sy-subrc.
    WHEN 1.
      ...
      PERFORM check_setid USING roiu_pr_dn-dn_set. "SO7K011423 BT
      IF l_line_count > 1.                          "SOGK005579 BT
        CLEAR roiu_pr_dn-dn_set.                        "SOGK005579 BT
      ENDIF.                                            "SOGK005579 BT
    WHEN OTHERS.
      CLEAR roiu_pr_dn-dn_set.
  ENDCASE.

ENDFORM.                      " DEFAULT_HIER_NODE
...
*KH SELECT * FROM oiu_cm_edit
   SELECT SINGLE * FROM oiu_cm_edit              "KH
   INTO l_oiu_cm_edit
       WHERE edit_no = '0009'.
*KH ENDSELECT.
...
* SOGK013053   EY   3/13/02   OSS 126627/2002   Hierarchy performance review
...
form CHECK_RAZORFISH_DN_DUPLICATE .
  ...
  DATA : lv_dn_no LIKE OIUH_PR_DN-DN_DN_NO ,
         lv_sub_dn LIKE   OIUH_PR_DN-DN_SUB_NWK_NO .
...
*      End of Note "N 1736401
    ...
    ENDIF.
  ENDIF.
endform.                     " CHECK_PREMAS_DN_DUPLICATE
